...
> "do dopamine-ux-07-theme". It is self-contained -- it needs no context from
> any other session.
>
> Canonical copy: `~/utils/dopamine-ux/07-dufs-theme.md` (source of truth; this is a
...
1. **Delete this file** and commit that deletion together with the
   implementation:
   ```bash
   git rm prompts/dopamine-ux-07-theme.md
   ```
...
2. **Log completion in the canonical index**, in `~/utils` (a *different* repo,
   ...
   #   | 07-dufs-theme.md | DONE <YYYY-MM-DD> | <impl commit sha> | <one-line note> |
   git add dopamine-ux/00-INDEX.md
   git commit -m "dopamine-ux: mark 07-dufs-theme.md done"
   git push
   ```
